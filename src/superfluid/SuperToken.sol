 ...
 */
contract SuperToken is Initializable, SuperfluidToken, ISuperToken {
    ...
    function initializeLogic(
        ...
        constantOutflowNFT = constantOutflowNFT_;
        constantInflowNFT = constantInflowNFT_;

        // // @note this initializes the NFT proxies the very first time
        // // that they are created
        // if (
        //     address(constantOutflowNFT_.superTokenLogic()) == address(0)
        //         || address(constantInflowNFT_.superTokenLogic()) == address(0)
        // ) {
        //     constantOutflowNFT_.initialize(_host, "Constant Outflow NFT", "COF");
        //     constantInflowNFT_.initialize(_host, "Constant Inflow NFT", "CIF");
        //     emit ConstantOutflowNFTCreated(constantOutflowNFT_);
        //     emit ConstantInflowNFTCreated(constantInflowNFT_);
        // }
    }
     ...
     * @param operatorData bytes extra information provided by the operator (if any)
     */
    function _burn(address operator, address from, uint256 amount, bytes memory userData, bytes memory operatorData)
        internal
    {
        if (from == address(0)) {
